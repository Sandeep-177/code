<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="../admin_css/commonAdmincss.css">
    <link rel="stylesheet" href="../admin_css/editSubCategory.css">
</head>
<body>
    <!-- Header -->
    <?php
    include 'adminHeader.php';
    ?>

    <!-- content -->
    <div class="divFist">
     <p class="fist" style="background-color: rgb(215, 248, 215); color: rgb(108, 143, 108);">Edit Subcategory</p>
     <div>
        <form id="orders">
        <label for="locEdt">Change Category</label>
        <input type="text" name="locEdt" class="locedt" id="input1">
        <label for="locEdt3">Edit Subcategory Name:</label>
        <input type="text" name="locEdt3" class="locedt" id="input3">
        <label for="locEdt4">Description:</label>
        <textarea name="locEdt4" class="locedt" id="input4"></textarea>
        <!-- <input type="text" name="locEdt4" class="locedt" id="input4"> -->
        <label for="locEdt2">Image:</label>
        <input type="file" name="locEdt2" class="locedt" id="input2">
        
        <div>
        <button id="update" class="btn">UPDATE</button>
        <button id="reset" class="btn">RESET</button>
        </div>
        </form>
     </div>
     <p class="fist" style="background-color: rgb(227, 233, 227); color: rgb(130, 136, 130);"><b>Note: </b> Please note that changes will be seeen by users and vendors.</p>

     </div>

    <!-- Footer -->
    <?php
    include 'adminFooter.php';
    ?>
    
</body>
</html>